<template>
  <div>
    <ECharts ref="map" class="map" :options="mapOption" />
    <div class="box" v-show="boxShow">
      <header>
        <h1>{{ boxInfo.title }}</h1>
      </header>
      <div class="content">
        <div class="list" v-for="(item, index) in boxInfo.data" :key="index">
...
export default {
  ...
  methods: {
    ...
    onTap(ev) {
      console.log(ev);
      let p = this.mapInstance.overlayPixelToPoint(
        new BMap.Pixel(ev.center.x, ev.center.y)
      );
      console.log(p);
    },
    onPanmove(ev) {
